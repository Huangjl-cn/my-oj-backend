...
public class JavaLanguageJudgeStrategy implements JudgeStrategy {
    ...
    public JudgeInfo doJudge(JudgeContext judgeContext) {
        ...
        //使用 Optional 来出来可能的NullPointerException
        ...
        Integer executeStatus = judgeContext.getExecuteStatus();
        if (executeStatus.equals(ExecuteStatusEnum.COMPILE_ERROR.getValue())) {
            judgeInfoResponse.setMessage(JudgeInfoMessageEnum.COMPILE_ERROR.getValue());
            return judgeInfoResponse;
        } else if (executeStatus.equals(ExecuteStatusEnum.RUNTIME_ERROR.getValue())) {
            judgeInfoResponse.setMessage(JudgeInfoMessageEnum.RUNTIME_ERROR.getValue());
            return judgeInfoResponse;
        } else if (executeStatus.equals(ExecuteStatusEnum.SYSTEM_ERROR.getValue())) {
        ...
        // 程序正常执行的情况下，先判断沙箱执行的结果输出数量是否和预期输出数量相等
